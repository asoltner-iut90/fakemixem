...
def generate_schedule(model_path, data_path, models_dir, steps=6):
    ...
    for i in range(steps):
        ...
        # Affichage console
        print(f"[{i+1}] {final_date.strftime('%d/%m')} - {dow_names[target_dow]} ({duration_min}min) : {clean_tags}")
...
def predict_one() -> dict:
    ...
    Returns:
        dict: Dictionnaire avec les détails de la vidéo prédite
    '''
...
def predict_multiple(nb) -> list:
    ...
    :param nb:
        int: Nombre de vidéos à prédire
    '''
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    model_path = os.path.join(BASE_DIR, 'models', 'planner_v5_hierarchical.keras')
    models_dir = os.path.join(BASE_DIR, 'models')
